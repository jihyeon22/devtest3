...
int send_uart_input_atcmd_err_msg = 0;
...
// uart thread
// ----------------------------------------
static int at_cust_uart_fd = AT_BRIDGE_UART_INVALID_HANDLE;

static int at_cust_uart_thread_run = 1;
static int uart_read_size = 1;

// *********************************************
// at uart api.
// *********************************************
int at_cust_uart_init()
{
	int ret;
	
	if ( (ret = mds_api_init_uart(UART_DEVICE_NAME, 115200)) < 0) 
	{
...
void at_cust_uart_deinit()
{

	if(at_cust_uart_fd != AT_BRIDGE_UART_INVALID_HANDLE)
	{
		mds_api_uart_close(at_cust_uart_fd);
		at_cust_uart_fd = AT_BRIDGE_UART_INVALID_HANDLE;
	}
...
int at_cust_write_uart(const char* buff, int buff_len, int send_cr)
{
    ...
    while ( write_cnt != p_send_buff_len )
    {
        ...
        ret = mds_api_uart_write(at_cust_uart_fd, p_send_buff, write_size );
        ...
        else
        {
            retry_cnt ++;
            usleep(2000*10);
            //LOGE(LOG_TARGET, "at_cust_write_uart error -1\r\n");
        }
...
int at_cust_atcmd_insert(char* output_buff, char* input_buff, int input_len)
{
	...
	for ( i = 0 ; i < input_len ; i++ )
	{
		...
		if (echo_setting == AT_BRIDGE_TRUE )
			at_cust_write_uart(&input_buff[i], 1, NO_SEND_CR);
...
void *thread_at_cust_uart(void *args)
{
	...
	while(at_cust_uart_thread_run)
	{
		memset(outbuf, 0x00, MAX_UART_READ_BUFF);
        
		uart_timeout_sec = UART_READ_TIMEOUT_SEC_ATCMD_MODE;
		readcnt = mds_api_uart_read(at_cust_uart_fd, outbuf , uart_read_size, uart_timeout_sec);
        
		if (readcnt <= 0) 
		{
			printf("uart %d sec timeout!\r\n", uart_timeout_sec);
            continue;
		}

		ret = at_cust_atcmd_insert(atcmd_buff, outbuf, readcnt);
...
int start_thread_at_cust_uart(void)
{
    ...
    if(pthread_create(&tid, &attr, thread_at_cust_uart, NULL) != 0)
    {
	...
	at_cust_uart_thread_run = 1;
    return 0;
}

void exit_thread_at_cust_uart(void)
{
	at_cust_uart_thread_run = 0;
}
